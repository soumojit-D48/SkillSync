...
class SummaryGenerator:
    """Generate AI-powered weekly summaries using Google Gemini."""
    ...
    def generate_ai_summary(self, week_data: dict) -> str:
        ...
        if self.gemini_model and settings.AI_SUMMARY_ENABLED:
            try:
                return self._generate_with_gemini(week_data)
            except Exception as e:
                logger.error(f"Gemini API failed: {e}. Falling back to template.")
...
Write the summary now:"""
        ...
        logger.info(f"Generated AI summary with {settings.GEMINI_MODEL}")
